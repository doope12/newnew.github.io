const allItems = {
	...
	id96: {
		weapon: "Butterly Knife",
		name: "Butterly Knife Crimson Web",
		skin: "Crimson Web",
		color: "gold",
	...
	id136: {
		...
		skin: "Doppler Ruby",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/talon/ruby.jpg",
		price: 4464.21,
		id: 136,
	},
	id137: {
	...
	id142: {
		...
		skin: "Doppler Black Pearl",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/butterfly/black-pearl.jpg",
		price: 9319.23,
		id: 142,
	},
	id143: {
	...
	id144: {
		weapon: "Karambit Knife",
		name: "Karambit Knife Crimson Web",
		skin: "Crimson Web",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/karambit/crimson-web.jpg",
		price: 4799.21,
		id: 144,
	},
	id145: {
	...
	id146: {
		weapon: "Karambit Knife",
		name: "Karambit Knife Gamma Doppler Emerald",
		skin: "Gamma Doppler Emerald",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/karambit/emerald.jpg",
		price: 11588,
		id: 146,
	},
	id147: {
	...
	id149: {
		weapon: "Karambit Knife",
		name: "Karambit Knife Doppler Ruby",
		skin: "Doppler Ruby",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/karambit/ruby.jpg",
		price: 9265.08,
		id: 149,
	},
	id150: {
		weapon: "Karambit Knife",
		name: "Karambit Knife Doppler Sapphire",
		skin: "Doppler Sapphire",
		color: "gold",
		imgDist: "../dist/img/weapons/knives/karambit/sapphire.jpg",
		price: 9016.92,
		id: 150,
	},
	id151: {
...
const addPlayer = () => {
	...
	playerItemsContainer.classList.add("jackpot__players-player-items-container");
	...
	playerAvatar.setAttribute(
		"src",
		`../dist/img/avatars/avatar${botPlayer}.jpg`
	);
...
const setJackpotChances = () => {
	...
	for (i = 0; i < 20; i++) {
		allChancesArray.forEach((item) => {
			const jackpotColor = document.createElement("div");
			jackpotColor.classList.add("jackpot__item");

			switch (item.parentElement.id) {
				case "jackpotRed":
					jackpotColor.classList.add("jackpot-red");
					jackpotColor.id = "redWin";
					break;
				case "jackpotBlue":
					jackpotColor.classList.add("jackpot-blue");
					jackpotColor.id = "blueWin";
					break;
				case "jackpotGreen":
					jackpotColor.classList.add("jackpot-green");
					jackpotColor.id = "greenWin";
					break;
				case "jackpotYellow":
					jackpotColor.classList.add("jackpot-yellow");
					jackpotColor.id = "yellowWin";
					break;
			}

			jackpotColor.style.width = `${parseFloat(item.textContent) * 10}px`;
			jackpotColors.append(jackpotColor);
		});
	}
};
...
const spinJackpot = () => {
	...
	setTimeout(() => {
		const redLineX = document
			.querySelector(".jackpot__middle-point")
			.getBoundingClientRect().x;

		function getWinningItem() {
			const items = document.querySelectorAll(".jackpot__item");
			let winningItem = null;

			items.forEach((item) => {
				const itemRect = item.getBoundingClientRect();
				const itemStartX = itemRect.x; // Pozycja startowa elementu
				const itemEndX = itemRect.x + itemRect.width; // Pozycja końcowa elementu

				// Sprawdź, czy redLineX znajduje się wewnątrz obszaru tego elementu
				if (redLineX >= itemStartX && redLineX <= itemEndX) {
					winningItem = item;
				}
			});
...
const addUserItems = () => {
	...
	for (i = 0; i < countItemsAmount; i++) {
		...
		for (j = 0; j < itemCount; j++) {
			...
			itemBox.classList.add("jackpot__playeritems-item");
			itemBox.classList.add(allItems[`id${i}`].color + "-drop2");
			itemImg.classList.add("jackpot__playeritems-item-img");
			itemName.classList.add("jackpot__playeritems-item-name");
			itemSkin.classList.add("jackpot__playeritems-item-skin");
			itemSkin.classList.add(allItems[`id${i}`].color + "-text");
			itemPrice.classList.add("jackpot__playeritems-item-price");
...
const getTotalValueOfPlayerItems = () => {
	...
	allItemsOfPlayerArray.forEach((item) => {
		if (item.classList.contains("active-item-jackpot")) {
			value += parseFloat(item.lastElementChild.textContent);
		}
	});
	...
	if (winningItem.id === playerCol) {
		for (i = 0; i < itemsToWin.length; i++) {
			if (
				...
				localStorage.getItem(`id${itemsToWin[i]}`) === NaN
			) {
				localStorage.setItem(`id${itemsToWin[i]}`, 1);
			} else {
				const itemToAdd =
					parseInt(localStorage.getItem(`id${itemsToWin[i]}`)) + 1;
				localStorage.setItem(`id${itemsToWin[i]}`, itemToAdd);
			}
		}
