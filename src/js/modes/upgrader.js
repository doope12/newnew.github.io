...
const upgraderBtn = document.querySelector(".upgrader__base-click");
...
const countPercent = () => {
	const infoBoxLeft = document.querySelector(".upgrader__infobox-left");
	const infoBoxRight = document.querySelector(".upgrader__infobox-right");
	const priceLeft = infoBoxLeft.firstElementChild.textContent.replace("$", "");
	const priceRight = infoBoxRight.firstElementChild.textContent.replace(
		"$",
		""
	);
...
const addWinningItems = (didWin) => {
	...
	if (didWin) {
		...
		if (
			...
			localStorage.setItem(`id${topRightBox.id}`, 1);
		} else {
			const itemToAdd =
...
const renderItems = () => {
	...
	} else if (userItemsArray.length > 0) {
